...
class DiscordGSM():
    ...
    async def on_ready(self):
        ...
        with open(icon_file_name, 'rb') as file:
            try:
                await bot.user.edit(username='GameServers', avatar=file.read())
            except:
                pass
    ...
    def get_embed(self, server):
        ...
        embed.set_footer(text=f'#MIP | Servers | Última actualización: ' + datetime.now().strftime('%a, %Y-%m-%d %I:%M:%S%p'), icon_url='https://github.com/eminetoffroad/DiscordGSM/raw/master/images/discordgsm.png')
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CheckAnyFailure):
        message = await ctx.send('''No tienes acceso a este comando!''')
